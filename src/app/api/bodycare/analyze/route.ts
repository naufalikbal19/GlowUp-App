...
import {
  buildBodycareRoutine,
  normalizeBodyConcerns,
  BODY_SKIN_TYPE_LABEL,
  BODY_CONCERN_LABEL,
  type BodySkinType,
  type BodyConcernKey,
} from "@/lib/bodycare/engine";

const questionnaireSchema = z.object({
  skinType: z.enum(["DRY", "OILY", "NORMAL", "SENSITIVE"]),
  concerns: z.array(
    z.enum(["KULIT_KERING", "KUSAM", "TEKSTUR_TIDAK_RATA", "STRETCH_MARK", "KULIT_AYAM", "AREA_GELAP", "JERAWAT_BADAN"])
  ),
});
...
export async function GET() {
  ...
  const analyses = await db.bodyAnalysis.findMany({
...
export async function POST(req: NextRequest) {
  ...
  let skinType: BodySkinType;
  let concernKeys: BodyConcernKey[];
  let summary: string;
  ...
  if (photo instanceof File && photo.size > 0) {
    ...
    const filename = `bodycare/${randomUUID()}.${ext}`;
    photoUrl = await uploadPhoto(buffer, filename, mediaType);

    const aiResult = await analyzePhotoWithAI("BODY", buffer.toString("base64"), mediaType);

    if (aiResult) {
      source = "ai_photo";
      skinType = matchSkinType(aiResult.skinType);
      concernKeys = normalizeBodyConcerns(aiResult.concerns);
    ...
    skinType = parsed.data.skinType;
    concernKeys = parsed.data.concerns;
    summary = `Jenis kulit tubuh: ${BODY_SKIN_TYPE_LABEL[skinType]}. Keluhan utama: ${
      concernKeys.length ? concernKeys.map((c) => BODY_CONCERN_LABEL[c]).join(", ") : "tidak ada keluhan spesifik"
    }.`;
  }

  const routine = buildBodycareRoutine(skinType, concernKeys);
  const severity = Object.fromEntries(concernKeys.map((c) => [c, "medium" as const]));

  const analysis = await db.bodyAnalysis.create({
    data: {
      profileId: profile.id,
      source,
      photoUrl,
      skinType: BODY_SKIN_TYPE_LABEL[skinType],
      concerns: concernKeys.map((c) => BODY_CONCERN_LABEL[c]),
      severity,
      summary,
...
function matchSkinType(raw: string): BodySkinType {
